...
public class InMemoryCacheBackend implements CacheBackend {
  ...
  static class HBOWeigher implements Weigher<String, Object> {
    // String: object header (16) + hash fields (8) + char[] ref (8) + char[] header (16) + padding.
    ...
    // ArrayList: object header (16) + size (4) + modCount (4) + elementData ref (8) + padding = 40
    private static final int ARRAYLIST_BASE_BYTES = 40;

    // Reference size (compressed OOPs)
    private static final int REFERENCE_BYTES = 8;

    // TPlanNodeRun: object header (16) + isset byte (1) + padding (7) + 5 longs (40) + list ref (8) = 72
    private static final int T_PLAN_NODE_RUN_BASE_BYTES = 72;

    // ArrayList<Long> for scan_input_rows: base (40) + refs (8*N) + Long objects (24*N) = 40 + 32*N
    private static final int SCAN_INPUT_ROWS_LIST_BASE_BYTES = 40;
    private static final int BYTES_PER_LONG_IN_LIST = 32;  // ref (8) + Long object (24)
    ...
    private static int weighHistoricalStatsValue(String key, HistoricalStatsValue<?> value) {
      ...
      for (Object run : runs) {
        ...
        if (run instanceof TPlanNodeRun) {
          List<Long> scanRows = ((TPlanNodeRun) run).getScan_input_rows();
          if (scanRows != null) {
            size += SCAN_INPUT_ROWS_LIST_BASE_BYTES
                + (long) BYTES_PER_LONG_IN_LIST * scanRows.size();
          }
        }
      }
