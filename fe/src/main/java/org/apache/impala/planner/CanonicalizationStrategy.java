 ...
 * - IGNORE_EQUALITY_CONSTANTS: Most aggressive, ignores all equality constants
 */
public enum CanonicalizationStrategy {
  ...
  /**
   * Removes constants from all equality predicates.
   * Most aggressive strategy, assumes predicate structure is more important
   * than specific values for matching similar queries.
   * Only point predicates (equality and IN) are canonicalized.
   *
   * Example: WHERE id = 100 AND name = 'foo' → WHERE id = <CONST> AND name = <CONST>
   * Non-example: WHERE id > 100 (range predicate, keeps constant)
   */
  IGNORE_EQUALITY_CONSTANTS(2);

  // Lower values indicate higher accuracy, higher values indicate more aggressive matching
  ...
  /**
   * Returns the error level of this strategy. Lower values indicate strategies
   * that preserve more information and have more accurate statistics matching.
   * Higher values indicate more aggressive canonicalization with potentially
   * less accurate statistics but higher cache hit rates.
   */
