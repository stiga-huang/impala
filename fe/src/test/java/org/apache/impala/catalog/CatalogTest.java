...
public class CatalogTest {
  ...
  public void testNumberOfGetFileStatusCalls() throws CatalogException, IOException {
    ...
    catalog_.invalidateTable(new TTableName("functional", "alltypes"),
        ...
        /*dbWasAdded=*/new Reference<Boolean>(), EventSequence.getUnusedTimeline());
    ...
    stats.reset();
    catalog_.reloadTable(table, "test", EventSequence.getUnusedTimeline());
    ...
    catalog_.reloadPartition(table, partitionSpec, new Reference<>(false),
        CatalogObject.ThriftObjectType.NONE, "test", EventSequence.getUnusedTimeline());
    assertEquals(0L, (long)opsCounts.getLong(GET_FILE_BLOCK_LOCS));
    ...
    assertEquals(0L, (long)opsCounts.getLong(GET_FILE_BLOCK_LOCS));
    stats.reset();
    catalog_.reloadTable(unpartTable, "test", EventSequence.getUnusedTimeline());
    assertEquals(0L, (long)opsCounts.getLong(GET_FILE_BLOCK_LOCS));
