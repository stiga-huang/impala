...
DEFINE_int32(catalog_operation_log_size, 100, "Number of catalog operations to retain "
    "in catalogd. If -1, the operation log has unbounded size.");
...
static void CatalogOpListToJson(const vector<TCatalogOpRecord>& catalog_ops,
  ...
  for (const auto& catalog_op : catalog_ops) {
    ...
    int64_t end_time_ms = catalog_op.finish_time_ms > 0 ?
