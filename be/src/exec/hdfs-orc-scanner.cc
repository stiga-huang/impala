...
orc::Literal HdfsOrcScanner::GetLiteralSearchArguments(ScalarExprEvaluator* eval,
  ...
  switch (type.type) {
    case TYPE_BOOLEAN:
      *predicate_type = orc::PredicateDataType::BOOLEAN;
      return GetOrcPrimitiveLiteral<bool, bool>(*predicate_type, val);
    case TYPE_TINYINT:
      *predicate_type = orc::PredicateDataType::LONG;
    ...
    case TYPE_FLOAT:
      *predicate_type = orc::PredicateDataType::FLOAT;
    ...
    case TYPE_TIMESTAMP: {
      *predicate_type = orc::PredicateDataType::TIMESTAMP;
      if (UNLIKELY(!val)) return orc::Literal(predicate_type);
      const TimestampValue* tv = reinterpret_cast<const TimestampValue*>(val);
      int64_t nanos;
      // Convert to Unix time in nanoseconds.
      if (UNLIKELY(!tv->UtcToUnixTimeLimitedRangeNanos(&nanos))) {
        int64_t millis;
        // Convert to Unix time in milliseconds. Returning null value in case we fail.
        if (UNLIKELY(!tv->FloorUtcToUnixTimeMillis(&millis))) {
          return orc::Literal(predicate_type);
        }
        return orc::Literal(millis / MILLIS_PER_SEC,
            (millis % 1000) * MICROS_PER_MILLI * NANOS_PER_MICRO);
      }
      return orc::Literal(nanos / NANOS_PER_SEC, nanos);
    }
    case TYPE_DATE: {
      *predicate_type = orc::PredicateDataType::DATE;
      if (UNLIKELY(!val)) return orc::Literal(predicate_type);
    ...
    case TYPE_DECIMAL: {
      *predicate_type = orc::PredicateDataType::DECIMAL;
