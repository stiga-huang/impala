...
#define CATCH_ORC_EXCEPTIONS(msg_format) \
  CATCH_ORC_EXCEPTIONS_WITH_STMT(msg_format, )
...
#define CATCH_ORC_EXCEPTIONS_WITH_STMT(msg_format, stmt)          \
...
Status HdfsOrcScanner::StartColumnReading(const orc::StripeInformation& stripe) {
  ...
  } CATCH_ORC_EXCEPTIONS("Encountered parse error in tail of ORC file $0: $1");
...
Status HdfsOrcScanner::NextStripe() {
  ...
  while (true) {
    ...
    } CATCH_ORC_EXCEPTIONS("Error in creating column readers for ORC file $0: $1.");
...
Status HdfsOrcScanner::AssembleRows(RowBatch* row_batch) {
  ...
  } CATCH_ORC_EXCEPTIONS("Encounter error in creating ORC row batch for file $0: $1.");
  ...
  while (continue_execution) {  // one ORC batch (ColumnVectorBatch) in a round
    if (orc_root_reader_->EndOfBatch()) {
      ...
      } CATCH_ORC_EXCEPTIONS_WITH_STMT(
          "Encounter parse error in ORC file $0: $1.", eos_ = true);
