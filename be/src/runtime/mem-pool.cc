...
bool MemPool::FindChunk(int64_t min_size, bool check_limits) noexcept {
  ...
  if (counters_) {
    ...
    counters_->sys_alloc_duration += duration;
    counters_->sys_alloc_max_duration = max(counters_->sys_alloc_max_duration, duration);
    counters_->max_chunk_size = max(counters_->max_chunk_size, chunk_size);
    counters_->total_allocated_bytes += chunk_size;
