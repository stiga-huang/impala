  ...
  def wait_for_page_ready(self, page_name, timeout=10, interval=1):
    ...
    while (total_wait < timeout):
      ...
      if response.status_code == requests.codes.not_found:
        LOG.info("Debug webpage /{} not yet available. Sleeping {}s before next retry"
                 .format(page_name, interval))
        sleep(interval)
        continue
      LOG.info("Debug webpage /{} is ready. total_wait: {}s"
               .format(page_name, total_wait))
      return
